...
def test_cpu(method: str, worker_num: int, period: int):
    ...
    stress = threading.Thread(target=perform_stress, args=(["stress-ng", "--cpu-method", method, "--cpu", str(worker_num), "--sched-period", str(period), "--timeout", str(measure_time + 10) + "s", "--metrics"],))
    stress.start()
    time.sleep(1)
