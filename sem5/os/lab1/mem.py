...
def test_mem(method: str, worker_num: int):
    ...
    stress = threading.Thread(target=perform_stress, args=(["stress-ng", "--" + method, str(worker_num), "--timeout", str(measure_time + 10) + "s", "--metrics"],))
    stress.start()
    time.sleep(1)
    ...
    plt.title("Memory: " + method + "; workers num = " + str(worker_num))
...
test_method("madvise", 9)
test_method("prefetch", 9)